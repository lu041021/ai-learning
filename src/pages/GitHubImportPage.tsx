...
export function GitHubImportPage() {
  ...
  const { phase, result, duplicate, setPhase, setResult, setDuplicate, setError, resetImport } = useImportStore()
  ...
  return (
    <div style={{ display: 'flex', gap: '20px', height: 'calc(100vh - 160px)' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto' }}>
          {repos.map((repo) => (
            <div
              ...
              style={{
                padding: '12px', borderRadius: 'var(--radius)',
                border: selectedRepo?.full_name === repo.full_name
                  ? '1px solid var(--accent)'
                  : '1px solid transparent',
                background: selectedRepo?.full_name === repo.full_name
                  ? 'var(--accent-light)'
                  : 'transparent',
      ...
      <div style={{
        ...
        {(rightPhase === 'links' || rightPhase === 'previewing' || rightPhase === 'importing' || rightPhase === 'done') && (
          <>
              ...
              {links.map((link) => (
                <div
                  ...
                  style={{
                    ...
                    border: selectedLink?.url === link.url
                      ? '1px solid var(--accent)'
                      : '1px solid var(--border)',
                    marginBottom: '6px',
                    background: selectedLink?.url === link.url
                      ? 'var(--accent-light)'
                      : 'var(--bg-primary)',
                  }}
                >
            ...
            {duplicate && (
              <div style={{
                ...
                <p style={{ fontSize: '14px', color: 'var(--warning)', margin: 0, marginBottom: '4px', fontWeight: 600 }}>
                  该链接已导入过
                </p>
                <p style={{ fontSize: '13px', color: 'var(--text-secondary)', margin: 0 }}>
                  课程: {duplicate.existing_course_title}
                </p>
                <button onClick={() => setDuplicate(null)} style={{
            ...
            {rightPhase === 'importing' && (
              <div style={{
                ...
                background: 'var(--accent-light)', display: 'flex', alignItems: 'center', gap: '10px',
            ...
            {rightPhase === 'done' && result && (
              <div style={{
                ...
                <div style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '10px', display: 'flex', gap: '12px' }}>
