...
function ActivityBarChart({
  ...
  data: { week: string; sessions: number; lessonsCompleted: number }[]
}) {
  if (data.length === 0)
    return (
      ...
      >
        {[
          ...
          { label: '准确率', value: `${data.accuracyPct}%`, color: 'var(--success)' },
          {
            ...
            sub: `最长 ${data.longestStreak} 天`,
            color: 'var(--warning)',
          },
          {
            label: '复习率',
            value: `${data.reviewRate.toFixed(1)}x`,
            sub: '每题平均',
            color: '#a78bfa',
          },
        ].map((card) => (
          ...
          >
            <div
              style={{ fontSize: '28px', fontWeight: 700, color: card.color, marginBottom: '4px' }}
            >
              {card.value}
            </div>
