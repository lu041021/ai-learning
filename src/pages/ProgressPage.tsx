...
export function ProgressPage() {
  ...
  const loadData = () => {
    ...
    Promise.all([
      ...
      api.getWrongAnswers(userId),
    ]).then(([d, wa]) => {
      if (mountedRef.current) { setData(d); setWrongAnswers(wa); setLoading(false) }
    }).catch(() => {
      if (mountedRef.current) { setError('加载进度数据失败'); setLoading(false) }
    })
  }
  ...
  if (error) {
    return (
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '40px', textAlign: 'center' }}>
        ...
        <button onClick={handleRefresh} style={{ padding: '8px 20px', background: 'var(--accent)', color: '#fff', borderRadius: 'var(--radius)', fontSize: '14px' }}>重试</button>
      </div>
    )
  }

  if (!data) {
    return <div style={{ color: 'var(--text-muted)', padding: '40px', textAlign: 'center' }}>暂无数据</div>
  }
    ...
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        ...
        <button
          ...
          style={{
            padding: '6px 14px', background: 'var(--bg-secondary)', color: 'var(--text-secondary)',
            borderRadius: 'var(--radius)', fontSize: '13px', border: '1px solid var(--border)',
          }}
        >
          刷新
        </button>
      ...
      <div style={{
        ...
        {[
          { label: '已完成课时', value: `${data.completed_lessons}/${data.total_lessons}`, color: 'var(--accent)' },
          { label: '已完成测验', value: String(data.total_quizzes), color: 'var(--warning)' },
          { label: '平均得分', value: data.total_quizzes > 0 ? `${avgPct}%` : '-', color: 'var(--success)' },
      ...
      <div style={{
        background: 'var(--bg-secondary)', borderRadius: 'var(--radius-lg)',
        border: '1px solid var(--border)', padding: '24px', marginBottom: '28px',
      }}>
        <h3 style={{ fontSize: '15px', fontWeight: 600, marginBottom: '16px', color: 'var(--text-primary)' }}>
          课程进度
        </h3>
        <div style={{ display: 'grid', gap: '16px' }}>
          {data.course_progress.map((cp) => {
            const pct = cp.total_lessons > 0 ? Math.round((cp.completed_lessons / cp.total_lessons) * 100) : 0
            return (
              <Link
                key={cp.course_id}
                to={`/courses/${cp.slug}`}
              ...
              <div key={courseSlug} style={{ marginBottom: '24px' }}>
                ...
                {items.map((wa, i) => (
                  <div key={i} style={{
                    ...
                    border: '1px solid var(--border)', padding: '16px', marginBottom: '10px',
                  }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', fontSize: '13px' }}>
                      ...
                      <div>
                        ...
                        <span style={{ color: 'var(--text-secondary)' }}>
                          {String.fromCharCode(65 + wa.correct_answer_index)}. {wa.options[wa.correct_answer_index]}
                        </span>
                      </div>
                    </div>
                    {wa.explanation && (
                      <div style={{ marginTop: '8px', fontSize: '12px', color: 'var(--text-muted)', lineHeight: 1.5, background: 'rgba(139, 92, 246, 0.06)', borderRadius: '6px', padding: '8px 12px' }}>
                        {wa.explanation}
                      </div>
                    )}
      ...
      {wrongAnswers.length === 0 && data.total_quizzes > 0 && (
        <div style={{
          background: 'var(--bg-secondary)', borderRadius: 'var(--radius-lg)',
          border: '1px solid var(--border)', padding: '20px', marginBottom: '28px',
          textAlign: 'center', color: 'var(--success)', fontSize: '14px',
        }}>
          全部正确！继续保持！
        </div>
      )}
  ...
  return (
    <div style={{ overflow: 'auto' }}>
      <div style={{ display: 'flex', gap: '3px' }}>
        <div style={{ display: 'grid', gridTemplateRows: 'repeat(7, 12px)', gap: '3px', marginRight: '4px' }}>
          {dayLabels.map((l, i) => (
            <div key={i} style={{ fontSize: '10px', color: 'var(--text-muted)', lineHeight: '12px', textAlign: 'right', width: '16px' }}>
              {l}
            </div>
          ))}
        </div>
        {weeks.map((week, wi) => (
          <div key={wi} style={{ display: 'grid', gridTemplateRows: 'repeat(7, 12px)', gap: '3px' }}>
            {week.map((day, di) => (
              <div
                key={di}
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '4px', marginTop: '8px', fontSize: '10px', color: 'var(--text-muted)' }}>
        少
        <span style={{ width: '10px', height: '10px', borderRadius: '2px', background: 'var(--bg-tertiary)' }} />
        <span style={{ width: '10px', height: '10px', borderRadius: '2px', background: 'rgba(59,130,246,0.3)' }} />
        <span style={{ width: '10px', height: '10px', borderRadius: '2px', background: 'rgba(59,130,246,0.55)' }} />
        <span style={{ width: '10px', height: '10px', borderRadius: '2px', background: 'rgba(59,130,246,0.85)' }} />
        多
      </div>
    </div>
  )
...
function KnowledgeTree({ tree }: { tree: TreeNodeData[] }) {
  const nodeW = 110; const layerGap = 130; const nodeH = 28; const vGap = 14

  const flatten = (nodes: TreeNodeData[], depth: number): { node: TreeNodeData; depth: number; row: number }[] => {
    let result: { node: TreeNodeData; depth: number; row: number }[] = []
    let row = 0
  ...
  if (flatNodes.length === 0) {
    return <div style={{ color: 'var(--text-muted)', fontSize: '13px', textAlign: 'center', padding: '20px 0' }}>暂无课程数据</div>
  }
    ...
    <div style={{ overflow: 'auto' }}>
      <svg width={svgW} height={svgH} style={{ minWidth: '100%' }}>
        ...
        {flatNodes.map(({ node, depth, row }) => {
          const x = depth * layerGap; const y = row * (nodeH + vGap)
