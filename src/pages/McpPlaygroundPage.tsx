...
export function McpPlaygroundPage() {
  ...
  const handleRun = () => {
    ...
    try { args = JSON.parse(requestBody) }
  ...
  const handleQuickDemo = async () => {
    ...
    for (const step of steps) {
      ...
      try {
        ...
        setResults((prev) => [...prev, { tool: step.tool.name, content: String(e), duration: 0, error: true }])
      }
    }
    setRunning(false)
  }
        ...
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 700, marginBottom: '4px' }}>
            ...
            <span style={{
              ...
              background: connected ? 'var(--success-light)' : connected === false ? 'var(--danger-light)' : 'var(--bg-tertiary)',
              color: connected ? 'var(--success)' : connected === false ? 'var(--danger)' : 'var(--text-muted)',
            }}>
      ...
      <div style={{ display: 'flex', gap: '16px', height: 'calc(100vh - 200px)', minHeight: '500px' }}>
          ...
          {tools.map((tool) => (
            <div
              ...
              style={{
                padding: '10px 16px',
                cursor: 'pointer',
                background: selectedTool?.name === tool.name ? 'var(--accent-light)' : 'transparent',
                borderLeft: selectedTool?.name === tool.name ? '3px solid var(--accent)' : '3px solid transparent',
                borderBottom: '1px solid var(--border)',
              }}
            >
                  ...
                  <div style={{ marginBottom: '12px' }}>
                    ...
                    {Object.entries(selectedTool.inputSchema.properties || {}).map(([key, schema]) => (
                      <div key={key} style={{ fontSize: '12px', marginBottom: '4px' }}>
                        ...
                        {schema.description && (
                          <span style={{ color: 'var(--text-secondary)', marginLeft: '8px' }}>{schema.description}</span>
                        )}
        ...
        <div style={{
          ...
          <div style={{
            ...
            {results.length > 0 && (
              <button
                onClick={() => navigator.clipboard.writeText(results.map((r) =>
                  `=== ${r.tool} (${r.duration}ms) ===\n${prettyJSON(r.content)}\n`
                ).join('\n'))}
                style={{
