...
pub fn run_migrations(conn: &Connection) -> Result<(), String> {
    ...
    // Learning path versioning: new table with version support
    conn.execute_batch(
    ...
    .map_err(|e| format!("learning_path_history migration: {}", e))?;
    ...
    .map_err(|e| format!("idx_active_path migration: {}", e))?;
    ...
    .map_err(|e| format!("search_index migration: {}", e))?;

    // Sync triggers: courses
    conn.execute_batch(
        ...
        END;"
    ).map_err(|e| format!("search trigger course insert: {}", e))?;
    ...
    conn.execute_batch(
        ...
        END;"
    ).map_err(|e| format!("search trigger course update: {}", e))?;
    ...
    conn.execute_batch(
        ...
        END;"
    ).map_err(|e| format!("search trigger course delete: {}", e))?;

    // Sync triggers: lessons
    conn.execute_batch(
        ...
        END;"
    ).map_err(|e| format!("search trigger lesson insert: {}", e))?;
    ...
    conn.execute_batch(
        ...
        END;"
    ).map_err(|e| format!("search trigger lesson update: {}", e))?;
    ...
    conn.execute_batch(
        ...
        END;"
    ).map_err(|e| format!("search trigger lesson delete: {}", e))?;

    // Sync triggers: quiz_questions
    conn.execute_batch(
        ...
        END;"
    ).map_err(|e| format!("search trigger question insert: {}", e))?;
    ...
    conn.execute_batch(
        ...
        END;"
    ).map_err(|e| format!("search trigger question update: {}", e))?;
    ...
    conn.execute_batch(
        ...
        END;"
    ).map_err(|e| format!("search trigger question delete: {}", e))?;
    ...
    .map_err(|e| format!("concepts migration: {}", e))?;
    ...
    .map_err(|e| format!("lesson_concepts migration: {}", e))?;
    ...
    if concept_count == 0 {
        let concepts = vec![
            ...
            "Deep Learning", "NLP", "Computer Vision", "AI Ethics",
            "Data Preprocessing", "Model Evaluation", "Feature Engineering",
            "Transfer Learning", "Generative AI",
        ];
        ...
        // Link concepts to lessons by keyword matching
        let mut l_stmt = conn
        ...
        for (lesson_id, lesson_title, lesson_content) in &lessons {
            let combined = format!("{} {}", lesson_title.to_lowercase(), lesson_content.to_lowercase());
