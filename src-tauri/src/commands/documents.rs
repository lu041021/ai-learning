...
pub fn upload_document(
    ...
    let text = doc_parser::extract_text_from_bytes(&file_bytes, &ext)?;
    let chunks = doc_parser::chunk_text(&text);
    let chunk_count = chunks.len() as i64;

    if chunk_count == 0 {
        return Err("文档中未提取到文本内容".to_string());
    }
...
#[tauri::command]
pub fn list_documents(user_id: i64, db: State<'_, DbPool>) -> Result<Vec<DocumentOut>, String> {
    let conn = db.get().map_err(|e| e.to_string())?;
    let mut stmt = conn
        .prepare(
...
#[tauri::command]
pub fn delete_document(doc_id: i64, user_id: i64, db: State<'_, DbPool>) -> Result<(), String> {
    let conn = db.get().map_err(|e| e.to_string())?;
    conn.execute(
